// SPDX-License-Identifier: BUSL-1.1
module.exports = async function ({ delay, isExecute }, { ethers: { getContract } }) {
  const { getPhase6ArgumentsUpdateRewards } = require("./getPhaseArguments");
  const timelock = await getContract("BigTimelockAdmin");

  if (!delay) {
    delay = await timelock.getMinDelay();
  }

  const args = await getPhase6ArgumentsUpdateRewards();
  if (isExecute) {
    const tx = await timelock.executeBatch(...args);
    await tx.wait();
    console.log("Update Reward Distributors executed");
  } else {
    args.push(delay);
    const tx = await timelock.scheduleBatch(...args);
    await tx.wait();
    console.log(`Update Reward Distributors scheduled in ${delay}s`);
  }
};
